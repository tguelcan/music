...
{#if title}
	<h1 class="title pb-4 pt-8">{title}</h1>
{/if}
<ul class="divide-y divide-gray dark:divide-primary-dark">
	{#each allTracks as track}
		<li
			class="flex items-top py-4 transition-all hover:bg-gray dark:hover:bg-primary-dark p-3"
			on:mousedown={() => ($preview_url = track.preview_url)}
		>
			{#if track?.album}
				<img
					alt="Artist"
